//! Implements discovering, opening, and interacting with vex devices connected over USB with async support.
//! This is the same as Asyncv5 but using async.

use tokio_serial::{SerialPortInfo, SerialPort};
use super::{VEX_USB_VID, VexPortType, VEX_V5_CONTROLLER_USB_PID, VEX_V5_BRAIN_USB_PID};

pub mod device;


/// The information of a Async vex serial port
#[derive(Clone, Debug)]
pub struct VexAsyncSerialPort {
...
#[derive(Clone, Debug)]
pub enum VexAsyncSerialDevice {
    V5Brain {
        user: VexAsyncSerialPort,
        system: VexAsyncSerialPort
    },
    V5Controller {
        system: VexAsyncSerialPort
    },
...
impl VexAsyncSerialDevice {
    pub async fn open(&self) -> Result<device::Device<Box<dyn SerialPort>, Box<dyn SerialPort>>, crate::errors::DeviceError> {
        ...
        match self {
            Self::V5Brain { user, system } => {
                ...
                let system_port = match tokio_serial::new(&system.port_info.port_name, 115200)
                ...
                // Open the user port
                let user_port = Some(match tokio_serial::new(&user.port_info.port_name, 115200)
                    .parity(tokio_serial::Parity::None)
                    .timeout(std::time::Duration::new(crate::devices::SERIAL_TIMEOUT_SECONDS, crate::devices::SERIAL_TIMEOUT_NS))
                    .stop_bits(tokio_serial::StopBits::One).open() {
                        Ok(v) => Ok(v),
...
/// Finds all Async vex v5 ports connected to the computer over usb.
fn find_async_ports() -> Result<Vec<VexAsyncSerialPort>, crate::errors::DeviceError> {
    ...
    let mut vex_ports: Vec<VexAsyncSerialPort> = Vec::new();
    ...
    for port in ports {
        ...
        if port_info.pid == VEX_V5_CONTROLLER_USB_PID {
            ...
            vex_ports.push(VexAsyncSerialPort { port_info: port, port_type: VexPortType::Controller });
...
/// Finds all Async V5 devices from their ports
pub fn find_async_devices() -> Result<Vec<VexAsyncSerialDevice>, crate::errors::DeviceError> {

    // Find all vex ports
    let ports = find_async_ports()?;
    // Create a vector of all vex devices
    let mut vex_devices = Vec::<VexAsyncSerialDevice>::new();
    ...
    while let Some(current_port) = port_iter.next() {
        ...
        if current_port.port_type == VexPortType::System {
            ...
            vex_devices.push(VexAsyncSerialDevice::V5Controller {
                system: current_port.clone()
            });
